...
class TestConfigCommandErrors:
    ...
    def test_config_help_command(self, temp_workspace_git_enabled, run_workspace):
        ...
        # Test config help
        result = run_workspace(["config", "help"], workspace_dir)
        assert result.returncode == 0
        assert "config set" in result.stdout
        assert "config show" in result.stdout
        assert "config import" in result.stdout
        assert "config set-default" in result.stdout
    ...
    def test_config_invalid_subcommand(self, temp_workspace_git_enabled, run_workspace):
        ...
        result = run_workspace(["config", "invalid"], workspace_dir)
        assert result.returncode != 0
    ...
    def test_config_no_subcommand(self, temp_workspace_git_enabled, run_workspace):
        ...
        # Test config without subcommand
        result = run_workspace(["config"], workspace_dir)
        assert result.returncode != 0
    ...
    def test_config_set_invalid_workspace(self, temp_workspace_git_enabled, run_workspace):
        ...
        result = run_workspace(["config", "set", "", "https://example.com/repo.git"], workspace_dir)
        assert result.returncode != 0
        
        # Test with workspace name containing invalid characters
        result = run_workspace(["config", "set", "../bad", "https://example.com/repo.git"], workspace_dir)
        assert result.returncode != 0
        
        # Test with workspace name containing spaces
        result = run_workspace(["config", "set", "my workspace", "https://example.com/repo.git"], workspace_dir)
    ...
    def test_config_set_malformed_url(self, temp_workspace_git_enabled, run_workspace):
        ...
        result = run_workspace(["config", "set", "test", "not a url"], workspace_dir)
        # Note: The tool might accept this as a local path
        
        # Test with empty URL
        result = run_workspace(["config", "set", "test", ""], workspace_dir)
        assert result.returncode != 0
        
        # Test with URL containing special characters
        result = run_workspace(["config", "set", "test", "https://example.com/repo with spaces.git"], workspace_dir)
    ...
    def test_config_set_invalid_ref(self, temp_workspace_git_enabled, run_workspace):
        ...
        result = run_workspace(["config", "set", "test", "https://example.com/repo.git", "", "..badref"], workspace_dir)
        # Git might allow this, but it's questionable
        
        # Test with empty ref (should be valid - means no pinning)
        result = run_workspace(["config", "set", "test", "https://example.com/repo.git", "", ""], workspace_dir)
        assert result.returncode == 0
    ...
    def test_config_import_nonexistent_file(self, temp_workspace_git_enabled, run_workspace):
        ...
        result = run_workspace(["config", "import", "test", "/non/existent/file.conf"], workspace_dir)
        assert result.returncode != 0
    ...
    def test_config_import_invalid_file(self, temp_workspace_git_enabled, run_workspace):
        ...
        # Test import
        result = run_workspace(["config", "import", "test", str(invalid_config)], workspace_dir)
    ...
    def test_config_import_empty_file(self, temp_workspace_git_enabled, run_workspace):
        ...
        result = run_workspace(["config", "import", "test", str(empty_config)], workspace_dir)
        assert result.returncode == 0
    ...
    def test_config_show_nonexistent_workspace(self, temp_workspace_git_enabled, run_workspace):
        ...
        result = run_workspace(["config", "show", "nonexistent"], workspace_dir)
    ...
    def test_config_show_corrupted_git_config(self, temp_workspace_git_enabled, run_workspace):
        ...
        result = run_workspace(["config", "show"], workspace_dir)
    ...
    def test_config_set_default_errors(self, temp_workspace_git_enabled, run_workspace):
        ...
        # Test with empty URL
        result = run_workspace(["config", "set-default", ""], workspace_dir)
        assert result.returncode != 0
        
        # Test with too many arguments
        result = run_workspace(["config", "set-default", "url", "branch", "ref", "extra"], workspace_dir)
...
class TestConfigEdgeCases:
    ...
    def test_config_with_special_characters(self, temp_workspace_git_enabled, run_workspace):
        ...
        show_result = run_workspace(["config", "show", "test"], workspace_dir)
    ...
    def test_config_with_very_long_values(self, temp_workspace_git_enabled, run_workspace):
        ...
        result = run_workspace(["config", "set", "test", long_url], workspace_dir)
        # Should handle long values appropriately
        
        # Create very long branch name
        long_branch = "feature/" + "b" * 200
        
        result = run_workspace(["config", "set", "test2", "https://example.com/repo.git", long_branch], workspace_dir)
    ...
    def test_config_concurrent_modifications(self, temp_workspace_git_enabled, run_workspace):
        ...
        def modify_config(workspace_name, url):
            run_workspace(["config", "set", workspace_name, url], workspace_dir)
        ...
        for i in range(5):
            result = run_workspace(["config", "show", f"workspace{i}"], workspace_dir)
            assert result.returncode == 0
    ...
    def test_config_inheritance_complex(self, temp_workspace_git_enabled, run_workspace):
        ...
        # Set default config
        run_workspace(["config", "set-default", "https://default.com/repo.git", "main"], workspace_dir)
        
        # Set workspace-specific partial override
        run_workspace(["config", "set", "test", "https://override.com/repo.git"], workspace_dir)
        ...
        result = run_workspace(["config", "show", "test"], workspace_dir)
        assert "override.com" in result.stdout or "https://override.com" in result.stdout
        
        # Test fallback to default
        result = run_workspace(["config", "show", "other"], workspace_dir)
...
""")
    ...
    def test_mixed_config_sources(self, temp_workspace_git_enabled, run_workspace):
        ...
        # Set default config
        run_workspace(["config", "set-default", "https://default.com/repo.git"], workspace_dir)
        
        # Set workspace-specific config
        run_workspace(["config", "set", "test", "https://specific.com/repo.git"], workspace_dir)
        
        # Create workspace and verify correct config is used
        result = run_workspace(["switch", "test"], workspace_dir)
...
""")
        ...
        result = run_workspace(["config", "import", "compat"], workspace_dir)
        assert result.returncode == 0
        
        # Show imported config
        show_result = run_workspace(["config", "show", "compat"], workspace_dir)
        assert result.returncode == 0