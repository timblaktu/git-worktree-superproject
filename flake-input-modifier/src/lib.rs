...
fn create_replacement_string_node(
    builder: &mut GreenNodeBuilder,
    new_value: &str,
) -> Result<(), String> {
    ...
    builder.token(
        rowan::SyntaxKind(SyntaxKind::TOKEN_STRING_START as u16),
        "\"",
    );
    builder.token(
        rowan::SyntaxKind(SyntaxKind::TOKEN_STRING_CONTENT as u16),
        new_value,
    );
